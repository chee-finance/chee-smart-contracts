...
contract PriceOracleProxy is PriceOracle {
    ...
    /// @notice Address of the cCelo contract, which has a constant price
    address public cCeloAddress;

    /// @notice Address of the cUSDC contract, which we hand pick a key for
    address public cUsdcAddress;

    /// @notice Address of the vUSDT contract, which uses the cUSDC price
     ...
     * @param cCeloAddress_ The address of cCELO, which will return a constant 1e18, since all prices relative to celo
     ...
     * @param vUsdtAddress_ The address of vUSDT, which uses the cUSDC price
     */
    constructor(address guardian_,
                address v1PriceOracle_,
                address cCeloAddress_,
    ...
    function getUnderlyingPrice(CToken cToken) public view returns (uint) {
        ...
        if (cTokenAddress == cCeloAddress) {
            // celo always worth 1
            return 1e18;
        }

        if (cTokenAddress == cUsdcAddress || cTokenAddress == vUsdtAddress) {
    ...
    function setSaiPrice(uint price) public {
        ...
        require(price < 0.1e18, "SAI price must be < 0.1 CELO");
        saiPrice = price;
    }
}
